...
<div class="row">
    ...
    <div class="col-md-4 mb-3">
        <label for="rfc" class="form-label">RFC del Proveedor</label>
...
<div class="row">
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="url" name="url_pruebas" id="url_pruebas" class="form-control @error('url_pruebas') is-invalid @enderror" value="{{ old('url_pruebas', $pac->url_pruebas) }}" required placeholder="https://sandbox.pac.com/v1/stamp">
...
<div class="row">
    <div class="col-md-6 mb-3">
        ...
        <input type="text" name="usuario" id="usuario" class="form-control @error('usuario') is-invalid @enderror" value="{{ old('usuario', $pac->usuario) }}" required>
        @error('usuario') <div class="invalid-feedback">{{ $message }}</div> @enderror
    </div>
    <div class="col-md-6 mb-3">
        <label for="password" class="form-label">Contraseña</label>
        <input type="password" name="password" id="password" class="form-control @error('password') is-invalid @enderror" placeholder="{{ $pac->exists ? 'Dejar en blanco para no cambiar' : '' }}">
        @error('password') <div class="invalid-feedback">{{ $message }}</div> @enderror
    </div>
</div>

<div class="form-check form-switch mb-3">
    <input class="form-check-input" type="checkbox" role="switch" id="is_active" name="is_active" value="1" @checked(old('is_active', $pac->is_active))>
    <label class="form-check-label" for="is_active">Activo</label>
</div>

<div class="text-end">
    <a href="{{ route('tenant.facturacion.pacs.index') }}" class="btn btn-secondary">Cancelar</a>
