        ...
        <div class="card-body">
            <form action="{{ route('admin.licencias.update', $licencia) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('tenant_id') is-invalid @enderror" id="tenant_id" name="tenant_id" required>
                        ...
                        @foreach ($tenants as $tenant)
                            <option value="{{ $tenant->id }}" {{ old('tenant_id', $licencia->tenant_id) == $tenant->id ? 'selected' : '' }}>
                                {{ $tenant->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('modulo_id') is-invalid @enderror" id="modulo_id" name="modulo_id" required>
                        ...
                        @foreach ($modulos as $modulo)
                            <option value="{{ $modulo->id }}" {{ old('modulo_id', $licencia->modulo_id) == $modulo->id ? 'selected' : '' }}>
                                {{ $modulo->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control @error('fecha_expiracion') is-invalid @enderror" id="fecha_expiracion" name="fecha_expiracion" value="{{ old('fecha_expiracion', $licencia->fecha_expiracion->format('Y-m-d')) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('max_usuarios') is-invalid @enderror" id="max_usuarios" name="max_usuarios" value="{{ old('max_usuarios', $licencia->max_usuarios) }}" min="1" required>
                    @error('max_usuarios')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('is_active') is-invalid @enderror" id="is_active" name="is_active" required>
                        <option value="1" {{ old('is_active', $licencia->is_active) == 1 ? 'selected' : '' }}>{{ __('Activa') }}</option>
                        <option value="0" {{ old('is_active', $licencia->is_active) == 0 ? 'selected' : '' }}>{{ __('Inactiva') }}</option>
                    </select>
                    @error('is_active')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="d-flex justify-content-end">
                    ...
                    <button type="submit" class="btn btn-primary">{{ __('Actualizar Licencia') }}</button>
                </div>
            </form>
        </div>
    </div>
